  ...
  return (
    <div className={`rounded-2xl p-5 border transition-all duration-300 flex flex-col justify-between relative overflow-hidden ${
      ...
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs font-bold px-2.5 py-1 rounded-lg bg-blue-100/80 text-blue-800 border border-blue-200">
          📅 {day.title}
        </span>
      ...
      {/* Details */}
      <div className="my-2">
        <div className="flex items-center text-sm font-bold text-slate-800 space-x-1.5">
          <span>🎯 {day.questionCount} கேள்விகள்</span>
          <span className="text-slate-400">•</span>
          <span className="text-slate-600">⏱ {day.durationMinutes} நிமிடம்</span>
        </div>
      ...
      <div className="mt-4">
        <button
          ...
          className={`w-full py-2.5 px-4 rounded-xl font-bold text-xs sm:text-sm flex items-center justify-center space-x-2 transition-colors ${
            isCompleted
              ? "bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm"
              : "bg-slate-900 hover:bg-blue-600 text-white shadow-sm"
          }`}
        >
          <span>{isCompleted ? "மீண்டும் எழுதுக" : "தொடங்கு"}</span>
          <span>→</span>
        </button>
      </div>
